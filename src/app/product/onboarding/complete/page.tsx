...
export default function OnboardingCompletePage() {
  ...
  return (
    <main className="min-h-screen bg-Budgexa-beige px-6 py-8 md:px-10 lg:px-16">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-7xl flex-col justify-between gap-10 lg:flex-row lg:items-center">
        <div className="w-full max-w-2xl">
          <div className="mb-10 flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-Budgexa-green text-white shadow-sm">
              <Wallet size={22} />
            </div>

            <div>
              <h2 className="text-xl font-semibold tracking-tight text-Budgexa-green">Budgexa</h2>
              <p className="text-sm text-Budgexa-green/60">Personal finance made simple</p>
            </div>
          </div>

          <div className="space-y-6">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-Budgexa-green/10 bg-white px-4 py-2 text-sm font-medium text-Budgexa-green shadow-sm">
                <BadgeCheck size={16} className="text-[#F5824A]" />
                Setup complete
              </div>

              <h1 className="max-w-xl text-4xl font-semibold leading-tight tracking-tight text-Budgexa-green md:text-5xl">
                Your financial workspace is ready.
              </h1>

              <p className="max-w-xl text-base leading-relaxed text-Budgexa-green/70 md:text-lg">
            ...
            <div className="space-y-4 pt-2">
              <div className="rounded-2xl border border-Budgexa-green/5 bg-white/70 p-4 backdrop-blur-sm">
                <p className="text-sm font-medium text-Budgexa-green">Selected goals</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {financialGoals.map((goal) => (
                    <span key={goal} className="rounded-full bg-Budgexa-beige/70 px-3 py-2 text-xs font-medium text-Budgexa-green">{goal}</span>
                  ))}
                </div>
              </div>

              <div className="rounded-2xl border border-Budgexa-green/5 bg-white/70 p-4 backdrop-blur-sm">
                <p className="text-sm font-medium text-Budgexa-green">Categories</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {categories.map((category) => (
                    <span key={category} className="rounded-full bg-white px-3 py-2 text-xs font-medium text-Budgexa-green shadow-sm">{category}</span>
                  ))}
                </div>
              </div>
            </div>

            <div className="mt-8 rounded-3xl bg-Budgexa-green p-5 text-white">
            ...
            <div className="flex flex-col gap-4 pt-4 sm:flex-row sm:items-center">
              <button
                ...
                disabled={saving}
                className="group inline-flex h-14 items-center justify-center gap-2 rounded-2xl bg-Budgexa-green px-7 text-base font-medium text-white transition-all hover:bg-[#1D3F1B] disabled:cursor-not-allowed disabled:opacity-60"
              >
        ...
        <div className="relative flex w-full max-w-xl items-center justify-center">
          ...
          <div className="relative w-full rounded-[32px] border border-Budgexa-green/5 bg-white p-6 shadow-xl shadow-Budgexa-green/5">
            <div className="mb-8 flex items-center justify-between">
              <div>
                <p className="text-sm text-Budgexa-green/50">Onboarding Snapshot</p>
                <h3 className="mt-1 text-3xl font-semibold tracking-tight text-Budgexa-green">Ready to launch</h3>
              </div>

              <div className="rounded-2xl bg-[#F5824A]/10 px-4 py-2 text-sm font-medium text-[#F5824A]">Complete</div>
            </div>

            <div className="mb-6 rounded-3xl bg-Budgexa-beige/60 p-5">
              <div className="mb-3 flex items-center justify-between">
                <p className="text-sm font-medium text-Budgexa-green">Profile</p>
                <p className="text-sm text-Budgexa-green/60">{levelLabel}</p>
              </div>
              ...
              <p className="mt-3 text-sm text-Budgexa-green/60">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={row.label} className="flex items-start justify-between gap-4 rounded-2xl border border-Budgexa-green/5 bg-Budgexa-green/[0.02] p-4">
                  <p className="text-sm text-Budgexa-green/50">{row.label}</p>
                  <p className="max-w-[60%] text-right text-sm font-medium text-Budgexa-green">{row.value}</p>
                </div>
              ))}
            </div>
          </div>
...
function SummaryCard({
  ...
  return (
    <div className="rounded-[28px] border border-Budgexa-green/5 bg-Budgexa-green/[0.02] p-5">
      <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-Budgexa-green/5 text-Budgexa-green">
        {icon}
      </div>
      ...
      <h3 className="mt-2 text-lg font-semibold text-Budgexa-green">
        {value}
      </h3>
    </div>
  );
