...
function ProgressBar({ value, color }: { value: number; color: string }) {
  ...
  return (
    <div className="h-1.5 w-full rounded-full bg-Budgexa-muted overflow-hidden">
      <div
...
function SummaryCard({ icon, label, value, sub }: { icon: React.ReactNode; label: string; value: string; sub: string }) {
  return (
    <div className="bg-white rounded-2xl border border-Budgexa-ash p-4 shadow-sm">
      <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-3 bg-Budgexa-beige-dark">{icon}</div>
      <p className="text-[11px] uppercase tracking-wide text-Budgexa-green/40">{label}</p>
      <p className="text-2xl font-bold text-Budgexa-green mt-1">{value}</p>
      <p className="text-xs text-Budgexa-green/45 mt-1">{sub}</p>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
        <h3 className="text-base font-semibold text-Budgexa-green">Delete budget?</h3>
        <p className="mt-2 text-sm text-Budgexa-green/60">
          The <span className="font-medium text-Budgexa-green">"{category}"</span> budget will be permanently removed. This cannot be undone.
        </p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-Budgexa-green">
            {initial ? "Edit Budget" : "New Budget"}
          </h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-Budgexa-green/40 hover:text-Budgexa-green transition">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="space-y-4">
          {/* Budget name */}
          <div>
            <label className="block text-xs font-medium text-Budgexa-green/60 mb-1.5">
            ...
            <input
              ...
              className="w-full h-11 px-3 rounded-xl border border-Budgexa-ash text-sm text-Budgexa-green bg-white focus:outline-none focus:ring-2 focus:ring-Budgexa-green/20"
            />
          </div>
          ...
          <div>
            ...
            {initial ? (
              <div className="h-11 px-3 rounded-xl border border-Budgexa-ash bg-Budgexa-muted text-sm text-Budgexa-green/50 flex items-center">
            ...
            {initial && (
              <p className="text-[11px] text-Budgexa-green/40 mt-1">Category cannot be changed after creation.</p>
            )}
          </div>

          {/* Monthly limit */}
          <div>
            <label className="block text-xs font-medium text-Budgexa-green/60 mb-1.5">Monthly Limit</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-Budgexa-green/40 font-medium">
              ...
              <input
                ...
                placeholder="0"
                className="w-full h-11 pl-7 pr-3 rounded-xl border border-Budgexa-ash text-sm text-Budgexa-green bg-white focus:outline-none focus:ring-2 focus:ring-Budgexa-green/20"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={onClose}
            className="flex-1 h-11 rounded-xl border border-Budgexa-ash text-sm text-Budgexa-green/60 hover:bg-Budgexa-muted transition">
            Cancel
          </button>
          <button onClick={handleSubmit} disabled={saving}
            className="flex-1 h-11 rounded-xl bg-Budgexa-green text-white text-sm font-medium flex items-center justify-center gap-2 hover:bg-Budgexa-green/90 transition disabled:opacity-50">
...
export default function BudgetPage() {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-Budgexa-beige/20 to-white rounded-2xl">
          ...
          <div className="flex flex-col 2xl:flex-row gap-6">
            ...
            <main className="flex-1 space-y-5 min-w-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                  <h1 className="text-3xl font-bold text-Budgexa-green">Monthly Budget</h1>
                  <p className="text-sm text-Budgexa-green/50 mt-1">Track, control, and optimize your spending.</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-2">
                  <DateRange />
                  <button
                    onClick={() => setModalTarget(null)}
                    className="h-11 px-4 rounded-xl bg-Budgexa-green text-white text-sm font-medium flex items-center justify-center gap-2 hover:bg-Budgexa-green/90 transition"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
                <SummaryCard label="Budgeted"    value={formatCurrency(totalBudgeted, currency)}  sub="Total allocation"            icon={<Wallet      size={16} className="text-Budgexa-green"  />} />
                <SummaryCard label="Spent"       value={formatCurrency(totalSpent, currency)}     sub={`${avgUsed}% of budget used`} icon={<ArrowUpRight size={16} className="text-Budgexa-red"    />} />
              ...
              {/* Table */}
              <div className="bg-white rounded-2xl border border-Budgexa-ash overflow-hidden shadow-sm">
                <div className="hidden lg:grid grid-cols-[2fr_1fr_1fr_2fr_1fr_auto] px-5 py-3 bg-Budgexa-muted text-[10px] text-Budgexa-green/40 uppercase tracking-wider">
                  <span>Category</span>
                ...
                {loading ? (
                  <div className="flex items-center justify-center py-16">
                    <Loader2 size={24} className="animate-spin text-Budgexa-green/40" />
                  </div>
                ) : budgets.length === 0 ? (
                  <div className="text-center py-16 text-sm text-Budgexa-green/40">
                  ...
                  <div className="divide-y divide-Budgexa-ash">
                    {budgets.map((b, i) => {
                      ...
                      return (
                        <div key={b.id} className="p-4 lg:p-5 hover:bg-Budgexa-muted/30 transition group">
                          ...
                          <div className="hidden lg:grid grid-cols-[2fr_1fr_1fr_2fr_1fr_auto] items-center">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-medium text-Budgexa-green">{b.name}</p>
                                {b.rollover && <p className="text-[11px] text-Budgexa-green/40">Rollover enabled</p>}
                              </div>
                            </div>
                            <span className="text-sm text-Budgexa-green">{formatCurrency(b.monthlyLimit, currency)}</span>
                            <span className="text-sm text-Budgexa-green">{formatCurrency(b.totalSpent, currency)}</span>
                            <div className="flex items-center gap-3 pr-4">
                              <ProgressBar value={b.percentUsed} color={color} />
                              <span className={cn("text-xs w-10 text-right", isOver ? "text-red-500 font-medium" : "text-Budgexa-green/60")}>
                            ...
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition w-16 justify-end">
                              <button
                                ...
                                className="p-1.5 rounded-lg text-Budgexa-green/40 hover:text-Budgexa-green transition"
                              >
                                <Pencil size={13} />
                              </button>
                              <button
                                onClick={() => setDeleteTarget(b)}
                                className="p-1.5 rounded-lg text-Budgexa-green/40 hover:text-red-500 transition"
                              >
                                <Trash2 size={13} />
                              </button>
                            </div>
                          ...
                          <div className="lg:hidden space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-1">
                                <button onClick={() => setModalTarget(b)} className="p-1.5 rounded-lg text-Budgexa-green/40 hover:text-Budgexa-green transition">
                                  <Pencil size={13} />
                                </button>
                                <button onClick={() => setDeleteTarget(b)} className="p-1.5 rounded-lg text-Budgexa-green/40 hover:text-red-500 transition">
                                  <Trash2 size={13} />
                                </button>
                              </div>
                            </div>
                            <ProgressBar value={b.percentUsed} color={color} />
                            <div className="grid grid-cols-3 text-xs text-Budgexa-green/60">
            ...
            <aside className="w-full 2xl:w-[340px] flex flex-col gap-4">
              <div className="rounded-2xl bg-gradient-to-br from-Budgexa-green to-Budgexa-green-dark text-white p-5">
                <p className="text-xs text-white/60 flex items-center gap-1"><Sparkles size={12} /> Budgexa Intelligence</p>
              ...
              <div className="bg-white border border-Budgexa-ash rounded-2xl p-5 shadow-sm space-y-3">
                <p className="text-sm font-semibold text-Budgexa-green">Budget Breakdown</p>
                {loading ? (
                  <div className="flex justify-center py-4">
                    <Loader2 size={20} className="animate-spin text-Budgexa-green/30" />
                  </div>
                ) : budgets.length === 0 ? (
                  <p className="text-xs text-Budgexa-green/40">No budgets to display.</p>
                ) : (
                  budgets.map((b, i) => (
                    <div key={b.id} className="space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-Budgexa-green/70">{b.name}</span>
                        <span className="text-xs text-Budgexa-green/50">{Math.round(b.percentUsed)}%</span>
                      </div>
